...
{
	const int power = value + 1;
	const int idx = ui->cbSampleRate->currentIndex();
	const int realPower = std::max(power, (idx - 1));
	m_settings.m_averagePeriodUS = (int)std::pow(10.0f, (float)realPower);
	ui->averagePeriodText->setText(m_averagePeriodTexts[value]);
	// ui->cbAveragePeriod->blockSignals(true);
	// ui->cbAveragePeriod->setCurrentIndex(realPower - 1);
	// ui->cbAveragePeriod->blockSignals(false);
	applySettings();
}

const QStringList HeatMapGUI::m_sampleRateTexts = {
	  "100S/s", "1kS/s", "10kS/s", "100kS/s", "1MS/s", "10MS/s"
};

void HeatMapGUI::on_sampleRate_valueChanged(int value)
{
	const int power = value + 2;
	const int idx = std::max(1, static_cast<int> (m_averagePeriodTexts.size()) - power);
	m_settings.m_sampleRate = (int)std::pow(10.0f, (float)power);
	ui->sampleRateText->setText(m_sampleRateTexts[value]); // value range is [2,7]
	ui->averagePeriod->setMinimum(idx);
	// ui->cbAveragePeriod->setCurrentIndex(std::max((idx - 1), ui->cbAveragePeriod->currentIndex()));
	...
	m_powerPathLossSeries(nullptr)
{
	...
	ui->cbSampleRate->addItems(m_sampleRateTexts);
	ui->cbAveragePeriod->addItems(m_averagePeriodTexts);
...
void HeatMapGUI::displaySettings()
{
	...
	ui->sbPulseTH->setValue(m_settings.m_pulseThreshold);
...
void HeatMapGUI::makeUIConnections()
{
	...
	QObject::connect(ui->cbSampleRate, QOverload<int>::of(&QComboBox::activated), this, &HeatMapGUI::on_sampleRate_valueChanged);
	QObject::connect(ui->cbAveragePeriod, QOverload<int>::of(&QComboBox::activated), this, &HeatMapGUI::on_averagePeriod_valueChanged);
	QObject::connect(ui->sbPulseTH, QOverload<int>::of(&QSpinBox::valueChanged), this, &HeatMapGUI::on_pulseTH_valueChanged);
...
void HeatMapGUI::hideScopeContainer() {
	ui->scopeContainer->hide();
	ui->scopeContainer->setDisabled(true);
	ui->glScope->hide();
	ui->glScope->setDisabled(true);
	ui->scopeGUI->hide();
	ui->scopeGUI->setDisabled(true);
	ui->sampleRate->hide();
	ui->averagePeriod->hide();
	ui->pulseTH->hide();
	ui->sampleRateText->hide();
	ui->averagePeriodText->hide();
	ui->pulseTHText->hide();
}
