...
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgConfigureGsmChannelyzer, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgReportChannels, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgStartScan, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgStopScan, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgScanComplete, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgScanResult, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgStatus, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgReportActiveFrequency, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgReportActivePower, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgReportScanning, Message)
MESSAGE_CLASS_DEFINITION(GsmChannelyzer::MsgReportScanRange, Message)

const char * const GsmChannelyzer::m_channelIdURI = "sdrangel.channel.gsmchannelyzer";
const char * const GsmChannelyzer::m_channelId = "GsmChannelyzer";

GsmChannelyzer::GsmChannelyzer(DeviceAPI *deviceAPI) :
        ...
        m_timeoutTimer(this)
{
    ...
    QObject::connect(
        ...
        this,
        &GsmChannelyzer::networkManagerFinished
    );
    QObject::connect(
        this,
        &ChannelAPI::indexInDeviceSetChanged,
        this,
        &GsmChannelyzer::handleIndexInDeviceSetChanged
    );

    start();

    QObject::connect(&m_availableChannelHandler, &AvailableChannelOrFeatureHandler::channelsOrFeaturesChanged, this, &GsmChannelyzer::channelsChanged);
    m_availableChannelHandler.scanAvailableChannelsAndFeatures();

    m_timeoutTimer.callOnTimeout(this, &GsmChannelyzer::timeout);
}

GsmChannelyzer::~GsmChannelyzer()
{
    qDebug("GsmChannelyzer::~GsmChannelyzer");
    QObject::disconnect(
        m_networkManager,
        &QNetworkAccessManager::finished,
        this,
...
void GsmChannelyzer::setDeviceAPI(DeviceAPI *deviceAPI)
{
...
uint32_t GsmChannelyzer::getNumberOfDeviceStreams() const
{
    return m_deviceAPI->getNbSourceStreams();
}

void GsmChannelyzer::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end, bool firstOfBurst)
{
    (void) firstOfBurst;
...
void GsmChannelyzer::start()
{
    ...
    qDebug("GsmChannelyzer::start");
    m_thread = new QThread();
    m_basebandSink = new GsmChannelyzerBaseband(this);
    ...
    GsmChannelyzerBaseband::MsgConfigureGsmChannelyzerBaseband *msg = GsmChannelyzerBaseband::MsgConfigureGsmChannelyzerBaseband::create(m_settings, QStringList(), true);
    m_basebandSink->getInputMessageQueue()->push(msg);

    m_running = true;
}

void GsmChannelyzer::stop()
{
    QMutexLocker m_lock(&m_mutex);
...
bool GsmChannelyzer::handleMessage(const Message& cmd)
{
    if (MsgConfigureGsmChannelyzer::match(cmd))
    {
        MsgConfigureGsmChannelyzer& cfg = (MsgConfigureGsmChannelyzer&) cmd;
        qDebug() << "GsmChannelyzer::handleMessage: MsgConfigureGsmChannelyzer";
    ...
    else if (DSPSignalNotification::match(cmd))
    {
        ...
        qDebug() << "GsmChannelyzer::handleMessage: DSPSignalNotification";
        // Forward to the sink
        if (m_running)
        {
...
void GsmChannelyzer::startScan()
{
    // Start scan
    m_state = START_SCAN;
}

void GsmChannelyzer::stopScan()
{
    // Stop scan
    m_state = IDLE;
    m_timeoutTimer.stop();
...
void GsmChannelyzer::setDeviceCenterFrequency(qint64 frequency)
{
...
void GsmChannelyzer::initScan()
{
    ...
    if (m_guiMessageQueue) {
        m_guiMessageQueue->push(GsmChannelyzer::MsgReportScanning::create());
    }

    m_state = SCAN_FOR_MAX_POWER;
}

void GsmChannelyzer::processScanResults(const QDateTime& fftStartTime, const QList<MsgScanResult::ScanResult>& results)
{
    switch (m_state)
    {
    case IDLE:
        ...
        if (fftStartTime >= m_minFFTStartTime)
        {
            ...
            if (complete)
            {
                if (m_scanResults.size() > 0)
                {
                    ...
                    if (m_guiMessageQueue)
                    {
                        GsmChannelyzer::MsgScanResult* msg = GsmChannelyzer::MsgScanResult::create(QDateTime());
                        QList<GsmChannelyzer::MsgScanResult::ScanResult>& guiResults = msg->getScanResults();
                        guiResults.append(m_scanResults);
                        m_guiMessageQueue->push(msg);
                    }

                    int frequency = -1;
                    GsmChannelyzerSettings::FrequencySettings *frequencySettings = nullptr;
                    GsmChannelyzerSettings::FrequencySettings *activeFrequencySettings = nullptr;

                    if (m_settings.m_priority == GsmChannelyzerSettings::MAX_POWER)
                    {
                    ...
                    if (m_settings.m_mode != GsmChannelyzerSettings::SCAN_ONLY)
                    {
                        ...
                        if (activeFrequencySettings)
                        {
                            ...
                            if (m_settings.m_mode == GsmChannelyzerSettings::SINGLE)
                            {
                                // Scan complete
                                if (m_guiMessageQueue) {
    ...
    case WAIT_FOR_END_TX:
        for (int i = 0; i < results.size(); i++)
        {
            if (results[i].m_frequency == m_activeFrequency)
            {
                ...
                GsmChannelyzerSettings::FrequencySettings *frequencySettings = m_settings.getFrequencySettings(m_activeFrequency);
                Real threshold = m_settings.getThreshold(frequencySettings);
...
void GsmChannelyzer::timeout()
{
    // Power hasn't returned above threshold - Restart scan
    initScan();
}

void GsmChannelyzer::calcScannerSampleRate(int channelBW, int basebandSampleRate, int& scannerSampleRate, int& fftSize, int& binsPerChannel)
{
...
void GsmChannelyzer::setCenterFrequency(qint64 frequency)
{
    GsmChannelyzerSettings settings = m_settings;
    settings.m_inputFrequencyOffset = frequency;
    applySettings(settings, {"inputFrequencyOffset"}, false);

    if (m_guiMessageQueue) // forward to GUI if any
    {
        MsgConfigureGsmChannelyzer *msgToGUI = MsgConfigureGsmChannelyzer::create(settings, {"inputFrequencyOffset"}, false);
        m_guiMessageQueue->push(msgToGUI);
    }
}

// Mute all channels
void GsmChannelyzer::muteAll(const GsmChannelyzerSettings& settings)
{
...
void GsmChannelyzer::applyChannelSetting(const QString& channel)
{
    if (!MainCore::getDeviceAndChannelIndexFromId(channel, m_scanDeviceSetIndex, m_scanChannelIndex)) {
        qDebug() << "GsmChannelyzer::applySettings: Failed to parse channel" << channel;
    }
}

void GsmChannelyzer::applySettings(const GsmChannelyzerSettings& settings, const QStringList& settingsKeys, bool force)
{
    qDebug() << "GsmChannelyzer::applySettings:"
    ...
    if (m_running)
    {
        GsmChannelyzerBaseband::MsgConfigureGsmChannelyzerBaseband *msg = GsmChannelyzerBaseband::MsgConfigureGsmChannelyzerBaseband::create(settings, settingsKeys, force);
        m_basebandSink->getInputMessageQueue()->push(msg);
    }
...
QByteArray GsmChannelyzer::serialize() const
{
    return m_settings.serialize();
}

bool GsmChannelyzer::deserialize(const QByteArray& data)
{
    if (m_settings.deserialize(data))
    {
        MsgConfigureGsmChannelyzer *msg = MsgConfigureGsmChannelyzer::create(m_settings, QStringList(), true);
...
int GsmChannelyzer::webapiSettingsGet(
...
int GsmChannelyzer::webapiWorkspaceGet(
...
int GsmChannelyzer::webapiSettingsPutPatch(
        bool force,
        ...
        QString& errorMessage)
{
    ...
    MsgConfigureGsmChannelyzer *msg = MsgConfigureGsmChannelyzer::create(settings, channelSettingsKeys, force);
    m_inputMessageQueue.push(msg);

    qDebug("GsmChannelyzer::webapiSettingsPutPatch: forward to GUI: %p", m_guiMessageQueue);
    if (m_guiMessageQueue) // forward to GUI if any
    {
        MsgConfigureGsmChannelyzer *msgToGUI = MsgConfigureGsmChannelyzer::create(settings, channelSettingsKeys, force);
        m_guiMessageQueue->push(msgToGUI);
    }
...
int GsmChannelyzer::webapiReportGet(
...
int GsmChannelyzer::webapiActionsPost(
        ...
        QString& errorMessage)
{
    ...
    {
        errorMessage = "Missing GsmChannelyzerActions in query";
        return 400;
    }
}

void GsmChannelyzer::webapiUpdateChannelSettings(
        GsmChannelyzerSettings& settings,
        const QStringList& channelSettingsKeys,
        SWGSDRangel::SWGChannelSettings& response)
{
    ...
    if (channelSettingsKeys.contains("frequencies"))
    {
        ...
        if (frequencies)
        {
            ...
            {
                GsmChannelyzerSettings::FrequencySettings freqSetting;
...
QList<SWGSDRangel::SWGFreqScannerFrequency *> *GsmChannelyzer::createFrequencyList(const GsmChannelyzerSettings& settings)
{
...
void GsmChannelyzer::webapiFormatChannelSettings(SWGSDRangel::SWGChannelSettings& response, const GsmChannelyzerSettings& settings)
{
...
void GsmChannelyzer::webapiFormatChannelReport(SWGSDRangel::SWGChannelReport& response)
{
...
void GsmChannelyzer::webapiReverseSendSettings(const QStringList& channelSettingsKeys, const GsmChannelyzerSettings& settings, bool force)
{
...
void GsmChannelyzer::webapiFormatChannelSettings(
        const QStringList& channelSettingsKeys,
        SWGSDRangel::SWGChannelSettings *swgChannelSettings,
        const GsmChannelyzerSettings& settings,
        bool force
)
{
    ...
    swgChannelSettings->setChannelType(new QString("GsmChannelyzer"));
...
void GsmChannelyzer::networkManagerFinished(QNetworkReply *reply)
{
    QNetworkReply::NetworkError replyError = reply->error();

    if (replyError)
    {
        qWarning() << "GsmChannelyzer::networkManagerFinished:"
    ...
    else
    {
        ...
        qDebug("GsmChannelyzer::networkManagerFinished: reply:\n%s", answer.toStdString().c_str());
    }

    reply->deleteLater();
}

void GsmChannelyzer::handleIndexInDeviceSetChanged(int index)
{
...
void GsmChannelyzer::channelsChanged(const QStringList& renameFrom, const QStringList& renameTo)
{
    m_availableChannels = m_availableChannelHandler.getAvailableChannelOrFeatureList();
    notifyUpdateChannels(renameFrom, renameTo);
}

void GsmChannelyzer::notifyUpdateChannels(const QStringList& renameFrom, const QStringList& renameTo)
{
    if (getMessageQueueToGUI())
    {
