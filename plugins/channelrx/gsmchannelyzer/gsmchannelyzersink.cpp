...
#include "gsmchannelyzersink.h"

GsmChannelyzerSink::GsmChannelyzerSink(GsmChannelyzer *ilsDemod) :
...
GsmChannelyzerSink::~GsmChannelyzerSink()
{
}

void GsmChannelyzerSink::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end)
{
    Complex ci;
...
void GsmChannelyzerSink::processOneSample(Complex &ci)
{
    ci /= SDR_RX_SCALEF;
    ...
    if (m_fftCounter == m_fftSize)
    {
        ...
        if (m_fftAverage.nextAverage())
        {
            ...
            {
                GsmChannelyzer::MsgScanResult* msg = GsmChannelyzer::MsgScanResult::create(m_fftStartTime);
                QList<GsmChannelyzer::MsgScanResult::ScanResult>& results = msg->getScanResults();
                ...
                for (int i = 0; i < m_settings.m_frequencySettings.size(); i++)
                {
                    if (m_settings.m_frequencySettings[i].m_enabled)
                    {
                        ...
                        if ((diff >= m_scannerSampleRate / 8) && (diff < m_scannerSampleRate * 7 / 8))
                        {
                            ...
                            Real power;
                            if (m_settings.m_measurement == GsmChannelyzerSettings::PEAK) {
                            ...
                            GsmChannelyzer::MsgScanResult::ScanResult result = {frequency, power};
...
Real GsmChannelyzerSink::totalPower(int bin, int channelBins) const
{
...
Real GsmChannelyzerSink::peakPower(int bin, int channelBins) const
{
    // Skip bin between halfway between channels
    // Then skip first and last bins, to avoid spectral leakage (particularly at DC)
    int startBin = bin - channelBins/2 + 1 + 1;
...
Real GsmChannelyzerSink::magSq(int bin) const
{
...
void GsmChannelyzerSink::applyChannelSettings(int channelSampleRate, int channelFrequencyOffset, int scannerSampleRate, int fftSize, int binsPerChannel, bool force)
{
    qDebug() << "GsmChannelyzerSink::applyChannelSettings:"
...
void GsmChannelyzerSink::applySettings(const GsmChannelyzerSettings& settings, const QStringList& settingsKeys, bool force)
{
    qDebug() << "GsmChannelyzerSink::applySettings:"
