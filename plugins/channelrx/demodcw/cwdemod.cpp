...
MESSAGE_CLASS_DEFINITION(CWDemod::MsgConfigureCWDemod, Message)

const char* const CWDemod::m_channelIdURI = "sdrangel.channel.cwdemod";
const char* const CWDemod::m_channelId = "CWDemod";

CWDemod::CWDemod(DeviceAPI *deviceAPI) :
        ...
        m_basebandSampleRate(0)
{
    ...
    QObject::connect(
        ...
        this,
        &CWDemod::networkManagerFinished
    );
    QObject::connect(
        this,
        &ChannelAPI::indexInDeviceSetChanged,
        this,
        &CWDemod::handleIndexInDeviceSetChanged
    );
}
...
void CWDemod::setDeviceAPI(DeviceAPI *deviceAPI)
{
...
uint32_t CWDemod::getNumberOfDeviceStreams() const
{
    return m_deviceAPI->getNbSourceStreams();
}

void CWDemod::setMessageQueueToGUI(MessageQueue* queue)
{
...
void CWDemod::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end, bool positiveOnly)
{
    (void) positiveOnly;
...
void CWDemod::start()
{
    ...
    qDebug() << "CWDemod::start";
    m_thread = new QThread();
    m_basebandSink = new CWDemodBaseband();
    ...
    CWDemodBaseband::MsgConfigureCWDemodBaseband *msg = CWDemodBaseband::MsgConfigureCWDemodBaseband::create(QStringList(), m_settings, true);
...
bool CWDemod::handleMessage(const Message& cmd)
{
    if (MsgConfigureCWDemod::match(cmd))
    {
        MsgConfigureCWDemod& cfg = (MsgConfigureCWDemod&) cmd;
        qDebug("CWDemod::handleMessage: MsgConfigureCWDemod");
    ...
    {
        qDebug() << "CWDemod::handleMessage: DSPSignalNotification";
    ...
    {
        qDebug() << "CWDemod::handleMessage: MsgChannelDemodQuery";
...
void CWDemod::setCenterFrequency(qint64 frequency)
{
    CWDemodSettings settings = m_settings;
    settings.m_inputFrequencyOffset = frequency;
    applySettings(QStringList("inputFrequencyOffset"), settings, false);

    if (m_guiMessageQueue) // forward to GUI if any
    {
        MsgConfigureCWDemod *msgToGUI = MsgConfigureCWDemod::create(QStringList("inputFrequencyOffset"), settings, false);
        m_guiMessageQueue->push(msgToGUI);
    }
}

void CWDemod::applySettings(const QStringList& settingsKeys, const CWDemodSettings& settings, bool force)
{
    qDebug() << "CWDemod::applySettings:" << settings.getDebugString(settingsKeys, force);
    ...
    if (m_running)
    {
        CWDemodBaseband::MsgConfigureCWDemodBaseband *msg = CWDemodBaseband::MsgConfigureCWDemodBaseband::create(settingsKeys, settings, force);
        m_basebandSink->getInputMessageQueue()->push(msg);
    }
...
QByteArray CWDemod::serialize() const
{
    return m_settings.serialize();
}

bool CWDemod::deserialize(const QByteArray& data)
{
    if (m_settings.deserialize(data))
    {
        MsgConfigureCWDemod *msg = MsgConfigureCWDemod::create(QStringList(), m_settings, true);
...
void CWDemod::sendSampleRateToDemodAnalyzer()
{
    QList<ObjectPipe*> pipes;
...
int CWDemod::webapiWorkspaceGet(
...
int CWDemod::webapiSettingsPutPatch(
        bool force,
        ...
        QString& errorMessage)
{
    ...
    MsgConfigureCWDemod *msg = MsgConfigureCWDemod::create(channelSettingsKeys, settings, force);
    m_inputMessageQueue.push(msg);

    qDebug("CWDemod::webapiSettingsPutPatch: forward to GUI: %p", m_guiMessageQueue);
    if (m_guiMessageQueue) // forward to GUI if any
    {
        MsgConfigureCWDemod *msgToGUI = MsgConfigureCWDemod::create(channelSettingsKeys, settings, force);
        m_guiMessageQueue->push(msgToGUI);
    }
...
void CWDemod::webapiUpdateChannelSettings(
        CWDemodSettings& settings,
        const QStringList& channelSettingsKeys,
        SWGSDRangel::SWGChannelSettings& response)
{
...
void CWDemod::webapiFormatChannelSettings(SWGSDRangel::SWGChannelSettings& response, const CWDemodSettings& settings)
{
...
void CWDemod::webapiFormatChannelReport(SWGSDRangel::SWGChannelReport& response)
{
...
void CWDemod::webapiReverseSendSettings(const QList<QString>& channelSettingsKeys, const CWDemodSettings& settings, bool force)
{
...
void CWDemod::sendChannelSettings(
    const QList<ObjectPipe*>& pipes,
    const QList<QString>& channelSettingsKeys,
    const CWDemodSettings& settings,
    bool force)
{
    qDebug("CWDemod::sendChannelSettings: %d pipes", (int)pipes.size());
...
void CWDemod::webapiFormatChannelSettings(
        const QList<QString>& channelSettingsKeys,
...
void CWDemod::networkManagerFinished(QNetworkReply *reply)
{
    QNetworkReply::NetworkError replyError = reply->error();

    if (replyError)
    {
        qWarning() << "CWDemod::networkManagerFinished:"
    ...
    else
    {
        ...
        qDebug("CWDemod::networkManagerFinished: reply:\n%s", answer.toStdString().c_str());
    }

    reply->deleteLater();
}

void CWDemod::handleIndexInDeviceSetChanged(int index)
{
