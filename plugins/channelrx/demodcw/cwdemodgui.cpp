...
#include "cwdemodgui.h"

#include "device/deviceuiset.h"

#include "ui_cwdemodgui.h"
...
CWDemodGUI* CWDemodGUI::create(PluginAPI* pluginAPI, DeviceUISet *deviceUISet, BasebandSampleSink *rxChannel)
{
	CWDemodGUI* gui = new CWDemodGUI(pluginAPI, deviceUISet, rxChannel);
	return gui;
}

void CWDemodGUI::destroy()
{
	delete this;
}

void CWDemodGUI::resetToDefaults()
{
	m_settings.resetToDefaults();
}

QByteArray CWDemodGUI::serialize() const
{
    return m_settings.serialize();
}

bool CWDemodGUI::deserialize(const QByteArray& data)
{
...
bool CWDemodGUI::handleMessage(const Message& message)
{
    if (CWDemod::MsgConfigureCWDemod::match(message))
    {
        qDebug("CWDemodGUI::handleMessage: CWDemod::MsgConfigureCWDemod");
        const CWDemod::MsgConfigureCWDemod& cfg = (CWDemod::MsgConfigureCWDemod&) message;
    ...
    {
        qDebug("CWDemodGUI::handleMessage: DSPConfigureAudio: %d", m_cwDemod->getAudioSampleRate());
    ...
    else if (DSPSignalNotification::match(message))
    {
        ...
        qDebug("CWDemodGUI::handleMessage: DSPSignalNotification: centerFrequency: %lld sampleRate: %d",
...
void CWDemodGUI::handleInputMessages()
{
    Message* message;
...
void CWDemodGUI::channelMarkerChangedByCursor()
{
...
void CWDemodGUI::channelMarkerHighlightedByCursor()
{
    setHighlighted(m_channelMarker.getHighlighted());
}

void CWDemodGUI::on_audioBinaural_toggled(bool binaural)
{
...
void CWDemodGUI::on_audioFlipChannels_toggled(bool flip)
{
...
void CWDemodGUI::on_dsb_toggled(bool dsb)
{
    ui->flipSidebands->setEnabled(!dsb);
    applyBandwidths(1 + ui->spanLog2->maximum() - ui->spanLog2->value());
}

void CWDemodGUI::on_deltaFrequency_changed(qint64 value)
{
...
void CWDemodGUI::on_BW_valueChanged(int value)
{
    (void) value;
    qDebug("CWDemodGUI::on_BW_valueChanged: ui->spanLog2: %d", ui->spanLog2->value());
    applyBandwidths(1 + ui->spanLog2->maximum() - ui->spanLog2->value());
}

void CWDemodGUI::on_lowCut_valueChanged(int value)
{
    (void) value;
    applyBandwidths(1 + ui->spanLog2->maximum() - ui->spanLog2->value());
}

void CWDemodGUI::on_volume_valueChanged(int value)
{
...
void CWDemodGUI::on_agc_toggled(bool checked)
{
...
void CWDemodGUI::on_agcClamping_toggled(bool checked)
{
    m_settings.m_agcClamping = checked;
    applySettings(QStringList("agcClamping"));
}

void CWDemodGUI::on_dnr_toggled(bool checked)
{
...
void CWDemodGUI::on_agcTimeLog2_valueChanged(int value)
{
...
void CWDemodGUI::on_agcPowerThreshold_valueChanged(int value)
{
...
void CWDemodGUI::on_agcThresholdGate_valueChanged(int value)
{
...
void CWDemodGUI::on_audioMute_toggled(bool checked)
{
	m_audioMute = checked;
...
void CWDemodGUI::on_spanLog2_valueChanged(int value)
{
...
void CWDemodGUI::on_flipSidebands_clicked(bool checked)
{
    (void) checked;
...
void CWDemodGUI::on_fftWindow_currentIndexChanged(int index)
{
...
void CWDemodGUI::on_filterIndex_valueChanged(int value)
{
    if ((value < 0) || (value >= 10)) {
        return;
    }
...
void CWDemodGUI::onMenuDialogCalled(const QPoint &p)
{
    if (m_contextMenuType == ContextMenuType::ContextMenuChannelSettings)
    {
        ...
        {
            dialog.setNumberOfStreams(m_cwDemod->getNumberOfDeviceStreams());
...
void CWDemodGUI::onWidgetRolled(QWidget* widget, bool rollDown)
{
    (void) widget;
    (void) rollDown;
...
CWDemodGUI::CWDemodGUI(PluginAPI* pluginAPI, DeviceUISet *deviceUISet, BasebandSampleSink *rxChannel, QWidget* parent) :
	ChannelGUI(parent),
	ui(new Ui::CWDemodGUI),
    ...
    m_fftNRDialog(nullptr)
{
    ...
    m_helpURL = "plugins/channelrx/demodcw/readme.md";
	...
	m_cwDemod = (CWDemod*) rxChannel;
    m_spectrumVis = m_cwDemod->getSpectrumVis();
	m_spectrumVis->setGLSpectrum(ui->glSpectrum);
	m_cwDemod->setMessageQueueToGUI(getInputMessageQueue());
    ...
    m_channelMarker.setTitle("CW Demodulator");
...
CWDemodGUI::~CWDemodGUI()
{
	delete ui;
}

bool CWDemodGUI::blockApplySettings(bool block)
{
...
void CWDemodGUI::applySettings(const QStringList& channelSettingsKeys, bool force)
{
	if (m_doApplySettings)
	{
        CWDemod::MsgConfigureCWDemod* message = CWDemod::MsgConfigureCWDemod::create(channelSettingsKeys, m_settings, force);
        m_cwDemod->getInputMessageQueue()->push(message);
	}
}

uint32_t CWDemodGUI::getValidAudioSampleRate() const
{
    // When not running, m_cwDemod->getAudioSampleRate() will return 0, but we
    // want a valid value to initialise the GUI, to allow a user to preselect settings
    int sr = m_cwDemod->getAudioSampleRate();
    if (sr == 0)
    {
...
unsigned int CWDemodGUI::spanLog2Max()
{
...
void CWDemodGUI::applyBandwidths(unsigned int spanLog2, bool force)
{
    ...
    qDebug() << "CWDemodGUI::applyBandwidths:"
            << " s2max:" << s2max
            << " dsb: " << dsb
            << " spanLog2: " << spanLog2
    ...
    ui->channelPowerMeter->setRange(CWDemodSettings::m_minPowerThresholdDB, 0);
...
void CWDemodGUI::displaySettings()
{
    m_channelMarker.blockSignals(true);
...
void CWDemodGUI::displayAGCPowerThreshold(int value)
{
    if (value == CWDemodSettings::m_minPowerThresholdDB)
    {
...
void CWDemodGUI::displayAGCThresholdGate(int value)
{
    QString s = QString::number(value, 'f', 0);
    ui->agcThresholdGateText->setText(s);
    int dialValue = value;
...
void CWDemodGUI::leaveEvent(QEvent* event)
{
	m_channelMarker.setHighlighted(false);
    ChannelGUI::leaveEvent(event);
}

void CWDemodGUI::enterEvent(EnterEventType* event)
{
	m_channelMarker.setHighlighted(true);
    ChannelGUI::enterEvent(event);
}

void CWDemodGUI::audioSelect(const QPoint& p)
{
...
void CWDemodGUI::dnrSetupDialog(const QPoint& p)
{
    m_fftNRDialog = new FFTNRDialog();
    m_fftNRDialog->move(p);
    QObject::connect(m_fftNRDialog, &FFTNRDialog::valueChanged, this, &CWDemodGUI::dnrSetup);
    ...
    m_fftNRDialog->exec();
    QObject::disconnect(m_fftNRDialog, &FFTNRDialog::valueChanged, this, &CWDemodGUI::dnrSetup);
    m_fftNRDialog->deleteLater();
    m_fftNRDialog = nullptr;
}

void CWDemodGUI::dnrSetup(int32_t iValueChanged)
{
    if (!m_fftNRDialog) {
        return;
    }
...
void CWDemodGUI::tick()
{
    double magsqAvg, magsqPeak;
    int nbMagsqSamples;
    m_cwDemod->getMagSqLevels(magsqAvg, magsqPeak, nbMagsqSamples);
    double powDbAvg = CalcDb::dbPower(magsqAvg);
    double powDbPeak = CalcDb::dbPower(magsqPeak);

    ui->channelPowerMeter->levelChanged(
            (CWDemodSettings::m_mminPowerThresholdDBf + powDbAvg) / CWDemodSettings::m_mminPowerThresholdDBf,
            (CWDemodSettings::m_mminPowerThresholdDBf + powDbPeak) / CWDemodSettings::m_mminPowerThresholdDBf,
            nbMagsqSamples);
    ...
    int audioSampleRate = m_cwDemod->getAudioSampleRate();
    bool squelchOpen = m_cwDemod->getAudioActive();
...
void CWDemodGUI::makeUIConnections()
{
    QObject::connect(ui->deltaFrequency, &ValueDialZ::changed, this, &CWDemodGUI::on_deltaFrequency_changed);
    QObject::connect(ui->audioBinaural, &QToolButton::toggled, this, &CWDemodGUI::on_audioBinaural_toggled);
    QObject::connect(ui->audioFlipChannels, &QToolButton::toggled, this, &CWDemodGUI::on_audioFlipChannels_toggled);
    QObject::connect(ui->dsb, &QToolButton::toggled, this, &CWDemodGUI::on_dsb_toggled);
    QObject::connect(ui->BW, &TickedSlider::valueChanged, this, &CWDemodGUI::on_BW_valueChanged);
    QObject::connect(ui->lowCut, &TickedSlider::valueChanged, this, &CWDemodGUI::on_lowCut_valueChanged);
    QObject::connect(ui->volume, &QDial::valueChanged, this, &CWDemodGUI::on_volume_valueChanged);
    QObject::connect(ui->agc, &ButtonSwitch::toggled, this, &CWDemodGUI::on_agc_toggled);
    QObject::connect(ui->agcClamping, &ButtonSwitch::toggled, this, &CWDemodGUI::on_agcClamping_toggled);
    QObject::connect(ui->dnr, &ButtonSwitch::toggled, this, &CWDemodGUI::on_dnr_toggled);
    QObject::connect(ui->agcTimeLog2, &QDial::valueChanged, this, &CWDemodGUI::on_agcTimeLog2_valueChanged);
    QObject::connect(ui->agcPowerThreshold, &QDial::valueChanged, this, &CWDemodGUI::on_agcPowerThreshold_valueChanged);
    QObject::connect(ui->agcThresholdGate, &QDial::valueChanged, this, &CWDemodGUI::on_agcThresholdGate_valueChanged);
    QObject::connect(ui->audioMute, &QToolButton::toggled, this, &CWDemodGUI::on_audioMute_toggled);
    QObject::connect(ui->spanLog2, &QSlider::valueChanged, this, &CWDemodGUI::on_spanLog2_valueChanged);
    QObject::connect(ui->flipSidebands, &QPushButton::clicked, this, &CWDemodGUI::on_flipSidebands_clicked);
    QObject::connect(ui->fftWindow, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &CWDemodGUI::on_fftWindow_currentIndexChanged);
    QObject::connect(ui->filterIndex, &QDial::valueChanged, this, &CWDemodGUI::on_filterIndex_valueChanged);
}

void CWDemodGUI::updateAbsoluteCenterFrequency()
{
