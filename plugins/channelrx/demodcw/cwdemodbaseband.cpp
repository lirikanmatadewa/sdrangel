...
#include "cwdemodbaseband.h"

MESSAGE_CLASS_DEFINITION(CWDemodBaseband::MsgConfigureCWDemodBaseband, Message)

CWDemodBaseband::CWDemodBaseband() :
    ...
    m_spectrumVis(nullptr)
{
    ...
    qDebug("CWDemodBaseband::CWDemodBaseband");
    QObject::connect(
        &m_sampleFifo,
        &SampleSinkFifo::dataReady,
        this,
        &CWDemodBaseband::handleData,
        Qt::QueuedConnection
    );
...
void CWDemodBaseband::setChannel(ChannelAPI *channel)
{
    m_sink.setChannel(channel);
}

void CWDemodBaseband::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end)
{
    m_sampleFifo.write(begin, end);
}

void CWDemodBaseband::handleData()
{
    QMutexLocker mutexLocker(&m_mutex);
...
void CWDemodBaseband::handleInputMessages()
{
	Message* message;
...
bool CWDemodBaseband::handleMessage(const Message& cmd)
{
    if (MsgConfigureCWDemodBaseband::match(cmd))
    {
        QMutexLocker mutexLocker(&m_mutex);
        MsgConfigureCWDemodBaseband& cfg = (MsgConfigureCWDemodBaseband&) cmd;
        qDebug() << "CWDemodBaseband::handleMessage: MsgConfigureCWDemodBaseband";
    ...
    else if (DSPSignalNotification::match(cmd))
    {
        ...
        qDebug() << "CWDemodBaseband::handleMessage: DSPSignalNotification: basebandSampleRate: " << notif.getSampleRate();
    ...
    else if (DSPConfigureAudio::match(cmd))
    {
        ...
        {
            qDebug("CWDemodBaseband::handleMessage: DSPConfigureAudio: new sample rate %d",audioSampleRate);
            ...
            {
                qDebug("CWDemodBaseband::handleMessage: DSPConfigureAudio: forward to GUI");
...
void CWDemodBaseband::applySettings(const QStringList& settingsKeys, const CWDemodSettings& settings, bool force)
{
...
int CWDemodBaseband::getChannelSampleRate() const
{
    return m_channelizer.getChannelSampleRate();
}


void CWDemodBaseband::setBasebandSampleRate(int sampleRate)
{
