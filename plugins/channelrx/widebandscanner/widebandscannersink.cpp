...
#include "widebandscannersink.h"

WidebandScannerSink::WidebandScannerSink(WidebandScanner *ilsDemod) :
        m_widebandScanner(ilsDemod),
        m_channel(nullptr),
...
WidebandScannerSink::~WidebandScannerSink()
{
}

void WidebandScannerSink::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end)
{
    Complex ci;
...
void WidebandScannerSink::processOneSample(Complex &ci)
{
    ci /= SDR_RX_SCALEF;
    ...
    if (m_fftCounter == m_fftSize)
    {
        ...
        if (m_fftAverage.nextAverage())
        {
            ...
            {
                WidebandScanner::MsgScanResult* msg = WidebandScanner::MsgScanResult::create(m_fftStartTime);
                QList<WidebandScanner::MsgScanResult::ScanResult>& results = msg->getScanResults();
                ...
                for (int i = 0; i < m_settings.m_frequencySettings.size(); i++)
                {
                    if (m_settings.m_frequencySettings[i].m_enabled)
                    {
                        ...
                        if ((diff >= m_scannerSampleRate / 8) && (diff < m_scannerSampleRate * 7 / 8))
                        {
                            ...
                            Real power;
                            if (m_settings.m_measurement == WidebandScannerSettings::PEAK) {
                            ...
                            WidebandScanner::MsgScanResult::ScanResult result = {frequency, power};
...
Real WidebandScannerSink::totalPower(int bin, int channelBins) const
{
...
Real WidebandScannerSink::peakPower(int bin, int channelBins) const
{
    // Skip bin between halfway between channels
    // Then skip first and last bins, to avoid spectral leakage (particularly at DC)
    int startBin = bin - channelBins/2 + 1 + 1;
...
Real WidebandScannerSink::magSq(int bin) const
{
...
void WidebandScannerSink::applyChannelSettings(int channelSampleRate, int channelFrequencyOffset, int scannerSampleRate, int fftSize, int binsPerChannel, bool force)
{
    qDebug() << "WidebandScannerSink::applyChannelSettings:"
...
void WidebandScannerSink::applySettings(const WidebandScannerSettings& settings, const QStringList& settingsKeys, bool force)
{
    qDebug() << "WidebandScannerSink::applySettings:"
