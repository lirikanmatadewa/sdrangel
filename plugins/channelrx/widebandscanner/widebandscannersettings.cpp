...
WidebandScannerSettings::WidebandScannerSettings() :
    m_channelMarker(nullptr),
    m_rollupState(nullptr)
{
    for (int i = 0; i < WIDEBANDSCANNER_COLUMNS; i++)
    {
...
void WidebandScannerSettings::resetToDefaults()
{
...
QByteArray WidebandScannerSettings::serialize() const
{
...
bool WidebandScannerSettings::deserialize(const QByteArray& data)
{
    ...
    if(d.getVersion() == 1)
    {
        ...
        d.readString(41, &m_title, "Wideband Scanner");
        if (m_channelMarker)
        {
...
void WidebandScannerSettings::applySettings(const QStringList& settingsKeys, const WidebandScannerSettings& settings)
{
...
QString WidebandScannerSettings::getDebugString(const QStringList& settingsKeys, bool force) const
{
...
QByteArray WidebandScannerSettings::FrequencySettings::serialize() const
{
    SimpleSerializer s(1);
...
bool WidebandScannerSettings::FrequencySettings::deserialize(const QByteArray& data)
{
    SimpleDeserializer d(data);
...
QDataStream& operator<<(QDataStream& out, const WidebandScannerSettings::FrequencySettings& settings)
{
    out << settings.serialize();
    return out;
}

QDataStream& operator>>(QDataStream& in, WidebandScannerSettings::FrequencySettings& settings)
{
    QByteArray data;
    in >> data;
...
QString WidebandScannerSettings::getChannel(WidebandScannerSettings::FrequencySettings *frequencySettings) const
{
...
Real WidebandScannerSettings::getThreshold(WidebandScannerSettings::FrequencySettings *frequencySettings) const
{
    ...
    if (!frequencySettings->m_threshold.isEmpty())
    {
        ...
        if (ok) {
            ...
            qDebug() << "WidebandScannerSettings::getThreshold: Failed to parse" << frequencySettings->m_threshold << "as a float";
        }
    }
    return threshold;
}

int WidebandScannerSettings::getChannelBandwidth(WidebandScannerSettings::FrequencySettings *frequencySettings) const
{
    ...
    if (!frequencySettings->m_channelBandwidth.isEmpty())
    {
        ...
        if (ok) {
            ...
            qDebug() << "WidebandScannerSettings::getChannelBandwidth: Failed to parse" << frequencySettings->m_channelBandwidth << "as an int";
        }
    }
    return channelBandwidth;
}

WidebandScannerSettings::FrequencySettings *WidebandScannerSettings::getFrequencySettings(qint64 frequency)
{
