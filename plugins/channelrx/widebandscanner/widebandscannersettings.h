...
#ifndef INCLUDE_WIDEBANDSCANNERSETTINGS_H
#define INCLUDE_WIDEBANDSCANNERSETTINGS_H
...
#define WIDEBANDSCANNER_COLUMNS           10

struct WidebandScannerSettings
{
    ...
    WidebandScannerSettings();
    void resetToDefaults();
    void setChannelMarker(Serializable *channelMarker) { m_channelMarker = channelMarker; }
    void setRollupState(Serializable *rollupState) { m_rollupState = rollupState; }
    QByteArray serialize() const;
    bool deserialize(const QByteArray& data);
    void applySettings(const QStringList& settingsKeys, const WidebandScannerSettings& settings);
    QString getDebugString(const QStringList& settingsKeys, bool force = false) const;
    QString getChannel(WidebandScannerSettings::FrequencySettings *frequencySettings) const;
    Real getThreshold(WidebandScannerSettings::FrequencySettings *frequencySettings) const;
    int getChannelBandwidth(WidebandScannerSettings::FrequencySettings *frequencySettings) const;
    WidebandScannerSettings::FrequencySettings *getFrequencySettings(qint64 frequency);
};

#endif /* INCLUDE_WIDEBANDSCANNERSETTINGS_H */
