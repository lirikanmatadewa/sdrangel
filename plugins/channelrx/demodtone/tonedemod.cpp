...
MESSAGE_CLASS_DEFINITION(ToneDemod::MsgConfigureWFMDemod, Message)

const char* const ToneDemod::m_channelIdURI = "sdrangel.channel.tonedemod";
const char* const ToneDemod::m_channelId = "ToneDemod";
const int ToneDemod::m_udpBlockSize = 512;

ToneDemod::ToneDemod(DeviceAPI* deviceAPI) :
	...
	m_basebandSampleRate(0)
{
	...
	QObject::connect(
		...
		this,
		&ToneDemod::networkManagerFinished
	);
	QObject::connect(
		this,
		&ChannelAPI::indexInDeviceSetChanged,
		this,
		&ToneDemod::handleIndexInDeviceSetChanged
	);
}
...
void ToneDemod::setDeviceAPI(DeviceAPI* deviceAPI)
{
...
uint32_t ToneDemod::getNumberOfDeviceStreams() const
{
	return m_deviceAPI->getNbSourceStreams();
}

void ToneDemod::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end, bool firstOfBurst)
{
	(void)firstOfBurst;
...
void ToneDemod::start()
{
	if (m_running) {
		return;
	}

	qDebug() << "ToneDemod::start";

	m_thread = new QThread(this);
	m_basebandSink = new ToneDemodBaseband();
	...
	ToneDemodBaseband::MsgConfigureWFMDemodBaseband* msg = ToneDemodBaseband::MsgConfigureWFMDemodBaseband::create(m_settings, true);
...
void ToneDemod::stop()
{
	...
	qDebug() << "ToneDemod::stop";
	m_running = false;
	m_thread->exit();
	m_thread->wait();
}

bool ToneDemod::handleMessage(const Message& cmd)
{
	if (MsgConfigureWFMDemod::match(cmd))
	{
		MsgConfigureWFMDemod& cfg = (MsgConfigureWFMDemod&)cmd;
		qDebug("ToneDemod::handleMessage: MsgConfigureWFMDemod");
	...
	else if (DSPSignalNotification::match(cmd))
	{
		...
		qDebug() << "ToneDemod::handleMessage: DSPSignalNotification";
	...
	{
		qDebug() << "ToneDemod::handleMessage: MsgChannelDemodQuery";
...
void ToneDemod::setCenterFrequency(qint64 frequency)
{
	ToneDemodSettings settings = m_settings;
...
void ToneDemod::applySettings(const ToneDemodSettings& settings, bool force)
{
	qDebug() << "ToneDemod::applySettings:"
	...
	if (m_running)
	{
		ToneDemodBaseband::MsgConfigureWFMDemodBaseband* msg = ToneDemodBaseband::MsgConfigureWFMDemodBaseband::create(settings, force);
		m_basebandSink->getInputMessageQueue()->push(msg);
	}
...
QByteArray ToneDemod::serialize() const
{
	return m_settings.serialize();
}

bool ToneDemod::deserialize(const QByteArray& data)
{
...
void ToneDemod::sendSampleRateToDemodAnalyzer()
{
	QList<ObjectPipe*> pipes;
...
int ToneDemod::webapiSettingsGet(
...
int ToneDemod::webapiWorkspaceGet(
...
int ToneDemod::webapiSettingsPutPatch(
	bool force,
	...
	QString& errorMessage)
{
	...
	qDebug("ToneDemod::webapiSettingsPutPatch: forward to GUI: %p", m_guiMessageQueue);
	if (m_guiMessageQueue) // forward to GUI if any
	{
...
void ToneDemod::webapiUpdateChannelSettings(
	ToneDemodSettings& settings,
	const QStringList& channelSettingsKeys,
	SWGSDRangel::SWGChannelSettings& response)
{
...
void ToneDemod::webapiFormatChannelSettings(SWGSDRangel::SWGChannelSettings& response, const ToneDemodSettings& settings)
{
...
void ToneDemod::webapiFormatChannelReport(SWGSDRangel::SWGChannelReport& response)
{
	if (!m_running) {
		return;
	}
...
void ToneDemod::webapiReverseSendSettings(QList<QString>& channelSettingsKeys, const ToneDemodSettings& settings, bool force)
{
...
void ToneDemod::sendChannelSettings(
	const QList<ObjectPipe*>& pipes,
	QList<QString>& channelSettingsKeys,
	const ToneDemodSettings& settings,
	bool force)
{
	for (const auto& pipe : pipes)
	{
...
void ToneDemod::webapiFormatChannelSettings(
	QList<QString>& channelSettingsKeys,
...
void ToneDemod::networkManagerFinished(QNetworkReply* reply)
{
	QNetworkReply::NetworkError replyError = reply->error();

	if (replyError)
	{
		qWarning() << "ToneDemod::networkManagerFinished:"
	...
	else
	{
		...
		qDebug("ToneDemod::networkManagerFinished: reply:\n%s", answer.toStdString().c_str());
	}

	reply->deleteLater();
}

void ToneDemod::handleIndexInDeviceSetChanged(int index)
{
