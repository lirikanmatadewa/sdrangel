...
#include "rdfdemodsink.h"

const unsigned int RDFDemodSink::m_rfFilterFftLength = 1024;

RDFDemodSink::RDFDemodSink() :
...
void RDFDemodSink::feed(const SampleVector::const_iterator& begin, const SampleVector::const_iterator& end)
{
	Complex ci;
	fftfilt::cmplx *rf;
	int rf_out;
			...
			if (m_interpolator.decimate(&m_interpolatorDistanceRemain, e, &ci))
			{
				...
				if(m_audioBufferFill >= m_audioBuffer.size())
				{
					...
					if (res != m_audioBufferFill) {
						qDebug("RDFDemodSink::feed: %lu/%lu audio samples written", res, m_audioBufferFill);
					}
...
void RDFDemodSink::applyAudioSampleRate(int sampleRate)
{
    if (sampleRate < 0)
    {
        qWarning("RDFDemodSink::applyAudioSampleRate: invalid sample rate: %d", sampleRate);
        return;
    }

    qDebug("RDFDemodSink::applyAudioSampleRate: %u", sampleRate);
...
void RDFDemodSink::applyChannelSettings(int channelSampleRate, int channelFrequencyOffset, bool force)
{
    qDebug() << "RDFDemodSink::applyChannelSettings:"
    ...
    {
        qDebug() << "RDFDemod::applyChannelSettings: m_interpolator.create";
        ...
        qDebug() << "RDFDemod::applySettings: m_rfFilter->create_filter";
        ...
        qDebug("RDFDemod::applySettings: m_fmExcursion: %f", m_fmExcursion);
    }
...
void RDFDemodSink::applySettings(const RDFDemodSettings& settings, bool force)
{
    qDebug() << "RDFDemodSink::applySettings:"
    ...
    {
        qDebug() << "RDFDemodSink::applySettings: m_interpolator.create";
        ...
        qDebug() << "RDFDemodSink::applySettings: m_rfFilter->create_filter";
        ...
        qDebug("RDFDemodSink::applySettings: m_fmExcursion: %f", m_fmExcursion);
    }

    if ((settings.m_squelch != m_settings.m_squelch) || force)
    {
        qDebug() << "RDFDemodSink::applySettings: set m_squelchLevel";
